...
public class ChessMatch {
    ...
    public ChessPiece perfomChessMove(ChessPosition sourcePosition, ChessPosition targetPosition) {
        ...
        if (movedPiece instanceof Pawn) {
            if ((movedPiece.getColor() == Color.WHITE && target.getRow() == 0) ||
                ...
                promotedPiece = replacePromotedPice("Q");
    ...
    public ChessPiece replacePromotedPice(String type) {
    ...
    private ChessPiece king(Color color) {
        List<Piece> list = piecesOnTheBoard.stream().filter(c -> ((ChessPiece) c).getColor() == color).collect(Collectors.toList());
        for (Piece p : list) {
    ...
    private boolean testCheck(Color color) {
        ...
        List<Piece> oppenentPieces = piecesOnTheBoard.stream().filter(c -> ((ChessPiece) c).getColor() == opponent(color)).collect(Collectors.toList());
        for (Piece p : oppenentPieces) {
    ...
    private boolean testCheckMate(Color color) {
        ...
        List<Piece> pieces = piecesOnTheBoard.stream().filter(c -> ((ChessPiece) c).getColor() == color).collect(Collectors.toList());
        for (Piece p : pieces) {
            boolean[][] mat = p.possibleMoves();
            for (int i = 0; i < board.getRows(); i++) {
                for (int j = 0; j < board.getColumns(); j++) {
